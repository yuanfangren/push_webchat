<template>
  <div class="content2">
    ...
    <div class="msg-edit-channel">
      本通道的订阅二维码
      <img :src="imgurl" alt>
    </div>
    <div class="edit-show-msg">
      可以通过「订阅消息API」发送微信给所有扫描过此二维码的人(共
      <span v-model="number"></span>人)
    </div>
  ...
  methods: {
    ...
    deleteChannel() {
      ...
      this.$confirm("确认删除该通道?", "提示", {
        ...
        .then(() => {
          axios
            .post(this.$api.deleteChannelById, {
              id: this.cid
            })
            .then(res => {
              if (res.status == 200) {
        ...
        .then(res => {
          ...
          if (res.status == 200) {
            if (res.data.status == 0) {
              ...
              this.subscribeList = res.data.data.subscribeList.map(i => i.nickname).join();
            } else {
              this.errorMessage(res.data.msg);
            }
          }
